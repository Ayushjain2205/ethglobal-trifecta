...
class AgentState(CopilotKitState):
    ...
    - mcp_config: to configure MCP services for the agent
    - use_custom_model: flag to switch between OpenAI and custom model
    """
    # Define mcp_config as an optional field without skipping validation
    mcp_config: Optional[MCPConfig]
    # Flag to switch between OpenAI and custom model
...
async def chat_node(state: AgentState, config: RunnableConfig) -> Command[Literal["__end__"]]:
    ...
    use_custom_model = state.get("use_custom_model", False)
    ...
    async with MultiServerMCPClient(mcp_config) as mcp_client:
        ...
        if use_custom_model:
            model = ChatOpenAI(
                ...
                openai_api_key="eyJhbGciOiJFUzI1NksiLCJ0eXAiOiJKV1QiLCJ3YWxsZXQiOiJNZXRhbWFzayJ9.eyJ1c2VyX2FkZHJlc3MiOiIweGNhZmE5M2U5OTg1NzkzZTI0NzViZDU4YjkyMTVjMjFkYmQ0MjFmZDAiLCJwdWJfa2V5IjoiUGdFbDZlNUFjWkFrdWZkZy9DWVJOYmU0ak5xSFpGWTZncnlxbXJubzJtcz0iLCJpYXQiOiIyMDI1LTAzLTIyVDE2OjIyOjQwLjE4NloiLCJleHAiOjE3NDUyNTI1NjB9.GYrL09WN/WbAV30kYAS+tdqpvpZSGhpQCs/plDjrKupF/eg/T7UBvXl55wlGCvajwjQoQqen9FlCsMrZZZz9WRs="
            )
        else:
            model = ChatOpenAI(model="gpt-4")
        
        # Create the react agent with the model
